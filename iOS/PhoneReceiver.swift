...
struct PerfStats: Equatable {
    ...
    var awdlChannel = 0           // current AWDL driver channel, 0 when unavailable
    var awdlBand = 0              // 1=2.4 GHz, 2=5 GHz, 3=6 GHz
    var awdlBandwidthMHz = 0
    var awdlFrequencyMHz = 0
    var awdlTxRateMbps = 0.0      // PHY Tx rate reported by CoreWiFi
    var awdlRxRateMbps = 0.0      // PHY Rx rate reported by CoreWiFi
    var awdlMaxLinkMbps = 0.0
    var awdlMCS = -1
    var awdlRSSI = 0
    var awdlPHYMode = -1
...
final class PhoneReceiver: ObservableObject {
    ...
    private var macAWDLChannel = 0
    private var macAWDLBand = 0
    private var macAWDLBandwidthMHz = 0
    private var macAWDLFrequencyMHz = 0
    private var macAWDLTxRateMbps = 0.0
    private var macAWDLRxRateMbps = 0.0
    private var macAWDLMaxLinkMbps = 0.0
    private var macAWDLMCS = -1
    private var macAWDLRSSI = 0
    private var macAWDLPHYMode = -1
    ...
    private func handleVideoChannelJSON(_ data: Data) {
        ...
        case "ping":
            ...
            macAWDLChannel = obj["awdlChannel"] as? Int ?? macAWDLChannel
            macAWDLBand = obj["awdlBand"] as? Int ?? macAWDLBand
            macAWDLBandwidthMHz = obj["awdlWidth"] as? Int ?? macAWDLBandwidthMHz
            macAWDLFrequencyMHz = obj["awdlFreq"] as? Int ?? macAWDLFrequencyMHz
            macAWDLTxRateMbps = (obj["awdlTx"] as? NSNumber)?.doubleValue ?? macAWDLTxRateMbps
            macAWDLRxRateMbps = (obj["awdlRx"] as? NSNumber)?.doubleValue ?? macAWDLRxRateMbps
            macAWDLMaxLinkMbps = (obj["awdlMax"] as? NSNumber)?.doubleValue ?? macAWDLMaxLinkMbps
            macAWDLMCS = obj["awdlMCS"] as? Int ?? macAWDLMCS
            macAWDLRSSI = obj["awdlRSSI"] as? Int ?? macAWDLRSSI
            macAWDLPHYMode = obj["awdlPHY"] as? Int ?? macAWDLPHYMode
    ...
    private func handleAnnexB(_ data: Data) {
        ...
        if let metaPrefix,
           let meta = try? JSONSerialization.jsonObject(with: metaPrefix) as? [String: Any],
           let rawCodec = meta["codec"] as? String,
           let codec = StreamCodec(rawValue: rawCodec), codec != streamCodec {
            streamCodec = codec
            vps = nil
            sps = nil
            pps = nil
        ...
        for nalu in nalus {
            ...
            if streamCodec == .hevc {
                ...
                case 39, 40: break // prefix/suffix SEI
        ...
        if formatDesc == nil {
            if streamCodec == .hevc, let vps, let sps, let pps {
                displayLayer.flush()
                buildHEVCFormatDescription(vps: vps, sps: sps, pps: pps)
            } else if streamCodec == .h264, let sps, let pps {
                displayLayer.flush()
                buildH264FormatDescription(sps: sps, pps: pps)
            }
        }
    ...
    private func finishFormatDescription(_ status: OSStatus) {
        if status == noErr, let formatDesc {
            let dims = CMVideoFormatDescriptionGetDimensions(formatDesc)
            Log.info("format description built: \(streamCodec.displayName) \(dims.width)x\(dims.height)")
            DispatchQueue.main.async {
                self.videoSize = CGSize(width: Int(dims.width), height: Int(dims.height))
            }
            setStatus("Receiving \(dims.width)×\(dims.height) · \(streamCodec.displayName)")
        } else {
            Log.info("format description FAILED: \(status) codec=\(streamCodec.displayName)")
        }
    }

    private func buildH264FormatDescription(sps: Data, pps: Data) {
    ...
    private func presentSample(_ sample: CMSampleBuffer,
        ...
        if elapsed >= 1.0 {
            ...
            stats.awdlChannel = macAWDLChannel
            stats.awdlBand = macAWDLBand
            stats.awdlBandwidthMHz = macAWDLBandwidthMHz
            stats.awdlFrequencyMHz = macAWDLFrequencyMHz
            stats.awdlTxRateMbps = macAWDLTxRateMbps
            stats.awdlRxRateMbps = macAWDLRxRateMbps
            stats.awdlMaxLinkMbps = macAWDLMaxLinkMbps
            stats.awdlMCS = macAWDLMCS
            stats.awdlRSSI = macAWDLRSSI
            stats.awdlPHYMode = macAWDLPHYMode
